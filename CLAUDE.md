...
- 構成: `main.py`(★エントリポイント/メインループ), `common/config.py`(★設定値の集約), `actuator/robot_io.py`(★Picoへの送信口・受信確証つき), `perception/`(MediaPipe手認識), `firmware/`(XIAO/Pico/ESP32), `ros2_bridge/`, `tools/`(手動確認スクリプト), `docker/`(未検証)
